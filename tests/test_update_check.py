...
class TestCheckForUpdateMessage:
    ...
    @patch("claude_swap.update_check.urllib.request.urlopen")
    def test_detected_method_non_windows_suggests_cshift_upgrade(
        self, mock_urlopen, tmp_path, monkeypatch
    ):
        # uv/pipx on macOS/Linux: cshift --upgrade actually upgrades, so advertise it.
        monkeypatch.setattr("claude_swap.update_check.CACHE_PATH", tmp_path / "cache.json")
        ...
        assert result is not None
        assert "cshift --upgrade" in result
    ...
    def test_detected_method_windows_suggests_direct_command(
        ...
        assert "cshift --upgrade" not in result

    @patch("claude_swap.update_check.urllib.request.urlopen")
    def test_unknown_method_suggests_cshift_instructions(
        self, mock_urlopen, tmp_path, monkeypatch
    ):
        # Unknown install method: cshift --upgrade can only show instructions.
        monkeypatch.setattr("claude_swap.update_check.CACHE_PATH", tmp_path / "cache.json")
        ...
        assert result is not None
        assert "cshift --upgrade` for upgrade instructions" in result
        assert "uv tool upgrade" not in result
